...
int main(int arg,char **argv){
    ...
    if(num_fat==32)num_clusters=4;
    else num_clusters=2;

    dataAreaPointer=lseek(fd,0,SEEK_CUR)/size_block-1;
    ...
    for(i=0;i<leftEntries;i++){
        ...
        if((entry.atributes != 0x0F)&&(entry.atributes&0x20)){
            ...
            cluster_32=entry.starting_cluster;
            printf("Cadena de clusters: %d ",cluster_32);
            dp_pointer = lseek(fd,0,SEEK_CUR);
            while(1){
                current= dataAreaPointer + cluster_32*2;
                lseek(fd,current,SEEK_SET);
                read(fd,&cluster_32,sizeof(cluster_32));
                if(cluster_32==-1)break;
                printf("%d ",cluster_32);
            }
            lseek(fd,dp_pointer,SEEK_SET);
            printf("Sali: \n");
        }
    }
...
//revisar tamaño bloques asignados a la hora de hacer el seek, desplazamiento 
//usar la técnica del patito