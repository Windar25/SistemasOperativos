...
void imprimirMONClustersLibres(int arg,char **argv,Fat16BootSector boot,int fd){
    int num_clusters,m,init_data_area,initFAT,n;
    short cluster,cluster_anterior,low_limit;
    //exit(1);
    if(arg==4){
        n=atoi(argv[3]);
        low_limit=n;
    ...
    //Inicio data area en sectores
    init_data_area = boot.reserved_sectors+boot.root_dir_entries*sizeof(FatEntry)/boot.sector_size + boot.fat_size_sectors;
    //obtenemos el espacio de la data area en sectores y vemos cuantos clusters pueden apuntar ahí
    num_clusters = (boot.total_sectors -init_data_area)/boot.sectors_per_cluster;


    initFAT =boot.reserved_sectors*boot.sector_size;

    printf("InitFat: %d\n",initFAT);
    printf("Reserved_sectors: : %d\n",boot.reserved_sectors);
    printf("Sector_size: %d\n",boot.sector_size);
        
    if(m>=num_clusters+2||m<0){
        printf("No se puede acceder al cluster, está fuera de los límites");
        exit(1);
    }

    int i = 2,free_cluster_printed=0,offset;
    
    for(;;){
        cluster_anterior=i;
        if(i>=num_clusters+2||free_cluster_printed==m)break;
        offset=initFAT+i*2;
        lseek(fd,offset,SEEK_SET);
        read(fd,&cluster,sizeof(cluster));
        if(cluster == 0){
            if(i>=low_limit){
                printf("El cluster %d no esta ocupado\n", cluster_anterior);
                free_cluster_printed++;
            }
        }
        i++;
    }
...
int main(int arg,char **argv){
    ...
    imprimirMONClustersLibres(arg,argv,boot16,fd);
